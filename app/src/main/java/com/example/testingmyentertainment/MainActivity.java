...
public class MainActivity extends AppCompatActivity {
    ...
    private void initializeLists() {
        ...
        for (int i = 0; i < courseLists.length; i++) {
            ...
            int endIndex = Math.min(startIndex + chunkSizes[i], size);
    ...
    private void setupGridViews() {
        AdapterView.OnItemClickListener listener = this::onGridItemClick;

        for (int i = 0; i < gridViews.length; i++) {
            String gridViewId = "idGVcourses" + (i + 1);
            int resId = getResources().getIdentifier(gridViewId, "id", getPackageName());
            gridViews[i] = findViewById(resId);
            gridViews[i].setAdapter(new CourseGVAdapter(this, courseLists[i]));
